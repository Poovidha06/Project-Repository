...
export default function ReportLost() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-14">
      <h1 className="font-display text-3xl font-bold mb-2">Report a <span className="text-gradient">Lost</span> Item</h1>
      <p className="text-muted mb-8">The more detail you give, the faster we can match it against found reports.</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="bg-base-light border border-white/5 rounded-xl2 p-8 flex flex-col gap-5">
        <div className="grid md:grid-cols-2 gap-5">
          <div>
            ...
            <input className={inputClass} placeholder="e.g. Navy Blue Water Bottle" {...register('title', { required: true })} />
          </div>
          <div>
        ...
        <div className="grid md:grid-cols-3 gap-5">
          <div>
            ...
            <input className={inputClass} placeholder="e.g. Milton" {...register('brand')} />
          </div>
          <div>
            <label className={labelClass}>Color</label>
            <input className={inputClass} placeholder="e.g. Blue" {...register('color')} />
          </div>
          <div>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input className={inputClass} placeholder="e.g. 2:00 PM" {...register('approximateTime')} />
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-5">
          ...
          <div>
            ...
            <input className={inputClass} placeholder="Main Academic Block" {...register('building')} />
          </div>
          <div>
            <label className={labelClass}>Floor</label>
            <input className={inputClass} placeholder="2nd Floor" {...register('floor')} />
          </div>
          <div>
            <label className={labelClass}>Room (optional)</label>
            <input className={inputClass} placeholder="204" {...register('room')} />
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-5">
          <div>
            ...
            <input className={inputClass} placeholder="e.g. ₹200" {...register('reward')} />
          </div>
          <div>
